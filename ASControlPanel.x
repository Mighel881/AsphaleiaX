...
+ (instancetype)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [sharedInstance setValue:[[NSData alloc] initWithBase64EncodedString:ImageData options:NSDataBase64DecodingIgnoreUnknownCharacters] forKey:@"smallIconData"];
...
- (void)load {
    if (!%c(LAActivator) || ![[%c(LAActivator) sharedInstance] isRunningInsideSpringBoard]) {
      return;
    }

    [[%c(LAActivator) sharedInstance] registerListener:self forName:@"Control Panel"];
}
...
- (void)unload {
    ...
    [[%c(LAActivator) sharedInstance] unregisterListenerWithName:@"Control Panel"];
}
